...
def main():
    ...
    ap.add_argument("--encoder", type=str, choices=["cnn","vit"], default="cnn")
    ...
    _, inter_val, inter_te = timestamp_split(inter, args.train_end, args.val_end, args.test_end)
    inter_tr = inter[(inter['timestamp'] <= args.train_end)].copy()
    ...
    model = TwoTower(num_users=len(user2idx), num_items=len(item2idx), id_dim=64, vis_dim=256)
