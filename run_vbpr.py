...
def main():
    ...
    inter_tr = apply_coldstart_policy(inter_tr, items, policy=args.coldstart-policy if False else args.coldstart_policy, fewshot_k=args.fewshot_k)  # avoid hyphen name issue

    # index encode after split to respect 0-shot (items unseen in train may get new idx; for skeleton we share mapping from all data)
    inter_all, items_idxed, user2idx, item2idx = build_indexers(inter, items)

    # remap splits to indices
    ...
    inter_val = inter_val[merge_cols].merge(inter_all[["user_id","item_id","timestamp","user_idx","item_idx"]], on=merge_cols, how="left")
    ...
    model = VBPR(num_users=len(user2idx), num_items=len(item2idx), id_dim=64, vis_dim=256)
